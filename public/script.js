...
/**
 * Helper: Formats markdown-like text to HTML
 */
function formatMarkdown(rawContent) {
  if (!rawContent) return "";
  
  // Trim and clean prefix
  let content = rawContent.replace(/^Success:\s*/i, '').trim();

  return content
    // 1. Convert headers (###, ##, #)
    .replace(/^### (.*$)/gim, '<h3 style="margin-top: 1rem; margin-bottom: 0.5rem; color: var(--accent-purple); font-size: 1.15rem; font-weight: 600;">$1</h3>')
    .replace(/^## (.*$)/gim, '<h3 style="margin-top: 1.1rem; margin-bottom: 0.55rem; color: var(--accent-purple); font-size: 1.2rem; font-weight: 600;">$1</h3>')
    .replace(/^# (.*$)/gim, '<h2 style="margin-top: 1.2rem; margin-bottom: 0.6rem; color: var(--accent-purple); font-size: 1.3rem; font-weight: 700;">$1</h2>')
    ...
    .replace(/^\s*[\*\-]\s+(.*$)/gim, '<ul style="margin: 0.4rem 0; padding-left: 1.2rem;"><li style="margin-bottom: 0.4rem; list-style-type: disc; line-height: 1.5;">$1</li></ul>')
    ...
    .replace(/<\/ul><br><ul style="margin: 0.4rem 0; padding-left: 1.2rem;">/g, '')
    .replace(/<\/ul>\s*<ul style="margin: 0.4rem 0; padding-left: 1.2rem;">/g, '');
}
...
function showExplanationModal(title, rawContent) {
  ...
  body.innerHTML = formatMarkdown(mainContent);
...
function startFlashcards() {
  ...
  studyFlashcardsData = parseFlashcards(activeGuideText);
  if (studyFlashcardsData.length === 0) {
    studyFlashcardsData.push({
...
function updateFlashcardUI() {
  ...
  if (studyFlashcardsData.length > 0) {
    qEl.innerText = studyFlashcardsData[activeFlashcardIndex].question;
    aEl.innerHTML = formatAIResponse(studyFlashcardsData[activeFlashcardIndex].answer);
    counter.innerText = `${activeFlashcardIndex + 1} / ${studyFlashcardsData.length}`;
  }
...
function nextFlashcard() {
  if (activeFlashcardIndex < studyFlashcardsData.length - 1) {
